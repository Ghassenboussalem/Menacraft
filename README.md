...
- **Reinforcement Learning Orchestrator**: Continuously improves bot detection accuracy by learning from feedback. When predictions are correct, agent weights are rewarded; when incorrect, they're adjusted to improve future performance.
...
Sara7a includes an intelligent RL orchestrator that learns from feedback to improve bot detection accuracy over time.

### How It Works

1. **Initial Weights**: Each detection agent starts with predefined weights based on signal strength
2. **Prediction**: System classifies accounts as 'bot' or 'real' using weighted agent scores
3. **Feedback**: When ground truth is provided, the system receives reward (+1) or punishment (-1)
4. **Learning**: Agent weights are adjusted based on their contribution to the decision
5. **Improvement**: Over time, the system learns which agents are most reliable for bot detection
...
Monitor RL performance and current agent weights:
...
Test the RL system with example scenarios:

```bash
node server/examples/rl-feedback-example.js
```

For detailed documentation, see [server/services/RL_README.md](server/services/RL_README.md)
