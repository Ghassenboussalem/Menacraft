...
try:
    ...
    from torchvision import transforms  # type: ignore[no-redef]
...
DEFAULT_MODEL_PATH = Path(__file__).parent / "models" / "mobilevit_best.pth"
...
class MobileViTClassifier:
    ...
    def __init__(self, model_path: str = None, device: str = None):
        """
        Args:
            model_path: Path to the .pth checkpoint. Defaults to models/mobilevit_best.pth
        ...
        if not os.path.exists(model_path):
            print(f"   [classifier] Model not found at {model_path} — classifier disabled")
            return
    ...
    def _load_model(self, model_path: str):
        ...
        # Use add_safe_globals (the intended 2.5.x API) so torch.load can deserialize
        # our trusted local checkpoint without upgrading to 2.6.
        import torch.serialization as _ts
        try:
            _ts.add_safe_globals([dict, list, set, tuple, int, float, str, bool])
        except AttributeError:
            pass  # torch < 2.4 doesn't have this API (and doesn't need it)

        # Load config from HF hub and override num_labels to match our 2-class checkpoint.
        # We must set id2label/label2id explicitly so the model doesn't complain about
        # the mismatch with the 1000-class ImageNet defaults.
        ...
        # Load our fine-tuned weights into the wrapper
        ...
        # Store the inner HF model for prediction
        self.model = wrapper
    ...
    def predict(self, image_path: str) -> dict | None:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Wrapper returns logits directly, HF model returns object
